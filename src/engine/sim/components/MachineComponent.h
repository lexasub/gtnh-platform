...
struct MachineComponent {
  ...
  bool managed_externally =
      false; // true = recipe handled by reciped, not MachineSystem
  ...
  MachineComponent(uint16_t type, uint32_t mb, uint32_t px, uint32_t py,
                   uint32_t pz, uint64_t mid)
      : machine_id(type), mb_id(mb), x(px), y(py), z(pz),
        machine_instance_id(mid) {};

  // Face roles: DOWN(0), UP(1), NORTH(2), SOUTH(3), WEST(4), EAST(5)
  void setFaceRole(uint8_t face, uint8_t role) {
    if (face < 6)
      side_config[face] = role;
  }
