...
// Lightweight item stack (matches RecipeManager::ItemStack and client ItemStack).
// Kept local to avoid dragging in large headers.
struct SlotItem {
    uint16_t item_id = 0;
    uint8_t  count   = 0;
    uint16_t meta    = 0;
...
template <size_t N, typename T = SlotItem>
class SlotContainer {
public:
    ...
    void set(int i, T v) {
        if (i < 0 || i >= static_cast<int>(N)) return;
        T old = slots_[i];
        slots_[i] = v;
        if (on_change_) on_change_(i, old, v);
    }

    void setAll(const std::array<T, N>& src) {
        for (size_t i = 0; i < N; ++i) set(static_cast<int>(i), src[i]);
    }

    void fill(const T& v) {
        for (size_t i = 0; i < N; ++i) set(static_cast<int>(i), v);
    }
    ...
    bool empty(int i) const { return i >= 0 && i < static_cast<int>(N) && slots_[i].item_id == 0; }
