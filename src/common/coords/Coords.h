...
inline uint64_t MakeChunkKey(ChunkCoord c) {
  uint64_t x =
      static_cast<uint64_t>(static_cast<int64_t>(c.x) + CHUNK_KEY_BIAS);
  uint64_t y =
      static_cast<uint64_t>(static_cast<int64_t>(c.y) + CHUNK_KEY_BIAS);
  uint64_t z =
      static_cast<uint64_t>(static_cast<int64_t>(c.z) + CHUNK_KEY_BIAS);
  return (x << 42) | (y << 21) | z;
}
...
inline ChunkCoord ChunkKeyToCoord(uint64_t key) {
  ChunkCoord c;
  c.x = static_cast<int32_t>(static_cast<int64_t>((key >> 42) & 0x1FFFFF) -
                             CHUNK_KEY_BIAS);
  c.y = static_cast<int32_t>(static_cast<int64_t>((key >> 21) & 0x1FFFFF) -
                             CHUNK_KEY_BIAS);
  c.z = static_cast<int32_t>(static_cast<int64_t>(key & 0x1FFFFF) -
                             CHUNK_KEY_BIAS);
  return c;
}
