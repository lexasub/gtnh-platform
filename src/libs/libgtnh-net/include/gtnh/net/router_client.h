...
// Router protocol message types — must match Go message_router/router.go MsgType
...
public:
    ...
    std::move_only_function<void(const std::string& topic,
                                  std::shared_ptr<std::vector<uint8_t>> data)> on_publish;
...
private:
    ...
    static std::vector<uint8_t> make_publish_frame(const std::string& topic,
                                                     const std::vector<uint8_t>& payload);
    static std::vector<uint8_t> make_register_frame(const std::string& service_name,
