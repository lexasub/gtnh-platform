...
struct WriteOp {
 ...
 // Thread-safe io_uring TCP connection.
  ...
  // CQE dispatch — called from connection's own poll loop.
    ...
    std::move_only_function<void(uint8_t type, const uint8_t* data, size_t len)> on_message;
...
private:
    ...
    bool grace_elapsed() const {
        return std::chrono::steady_clock::now() - connect_start_ > std::chrono::milliseconds(2000);
    }
