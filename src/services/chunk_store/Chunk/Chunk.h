...
struct alignas(64) Chunk {
    ...
    alignas(64) mutable std::array<uint8_t,  VOLUME> meta;    // orientation, mb_id lower 8 bits
    alignas(64) mutable std::array<uint32_t, VOLUME> multiblock; // full mb_id, 0 = free
    ...
    std::array<uint16_t, VOLUME>& getBlocks() const {
        return blocks;
    }
