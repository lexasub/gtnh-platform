...
template <typename Value, size_t Capacity> class ClockCache {
...
#pragma GCC diagnostic pop
  ...
  static bool cas(PackedSlot *p, PackedSlot *exp, PackedSlot des) noexcept {
    return __atomic_compare_exchange_n(p, exp, des, false, __ATOMIC_ACQ_REL,
                                       __ATOMIC_RELAXED);
  }
  static bool cas_relaxed(PackedSlot *p, PackedSlot *exp,
                          PackedSlot des) noexcept {
    return __atomic_compare_exchange_n(p, exp, des, false, __ATOMIC_RELAXED,
                                       __ATOMIC_RELAXED);
  }
  ...
  static int64_t unpack_key(PackedSlot s) noexcept {
    return static_cast<int64_t>(s >> 64);
  }
  static uintptr_t unpack_val(PackedSlot s) noexcept {
    return static_cast<uintptr_t>(s) & ~kRefBit;
  }
  static bool has_ref(PackedSlot s) noexcept {
    return static_cast<uintptr_t>(s) & kRefBit;
  }
  static PackedSlot set_ref(PackedSlot s) noexcept {
    return s | static_cast<PackedSlot>(kRefBit);
  }
  static PackedSlot clear_ref(PackedSlot s) noexcept {
    return s & ~static_cast<PackedSlot>(kRefBit);
  }
...
public:
  ...
  void put(int64_t key, Value value) noexcept {
    ...
    for (;;) {
      ...
      store_hand(&hands_[si], static_cast<uint8_t>((h + 1) & (kAssoc - 1)));
    }
  }
