...
public:
  ...
  static void buildSubscribeFrame(std::vector<uint8_t> &out,
  ...
  static void buildPublishFrame(std::vector<uint8_t> &out,
                                const std::string &topic, const uint8_t *data,
                                size_t data_len) {
  ...
  static void buildRegisterFrame(std::vector<uint8_t> &out,
                                 const std::string &name,
                                 const std::vector<std::string> &topics) {
    size_t payload_len = 1 + 2 + name.size() + 2;
    for (const auto &t : topics)
      payload_len += 2 + t.size();
    out.resize(4 + payload_len);
    writeBE32(out.data(), payload_len);
