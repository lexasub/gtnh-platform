...
public:
    ...
    void registerMachine(uint64_t machineEntityId, int32_t x, int32_t y, int32_t z);
    ...
    const std::vector<ExplodedNode>& getExplodedNodes() const { return m_explodedThisTick; }
...
private:
    ...
    std::vector<uint64_t> findPath(uint64_t fromCableNode, uint64_t toMachineNode);
};
