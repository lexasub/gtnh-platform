...
public:
    ...
    bool addNodeWithId(uint64_t id, int32_t x, int32_t y, int32_t z, uint16_t blockId);
    ...
    // Returns map of node_id -> energy_delta (positive = received, negative = sent)
    std::unordered_map<uint64_t, int32_t> distributeEnergy(uint64_t networkId, int32_t tickEnergy);

    // Distribute fluid across a network for one tick
    std::unordered_map<uint64_t, int32_t> distributeFluid(uint64_t networkId, int32_t tickFluid);
    ...
    void setNodeEnergy(uint64_t nodeId, int32_t energy, int32_t capacity, bool isSource, bool isSink);
    void setNodeFluid(uint64_t nodeId, int32_t fluid, int32_t capacity, uint32_t fluidId, bool isSource, bool isSink);
    void setNodeItemProps(uint64_t nodeId, uint8_t itemCapacity, bool isItemSource);
...
private:
    ...
    std::unordered_map<uint64_t, uint64_t> nodeToNetwork_;  // node_id -> network_id
