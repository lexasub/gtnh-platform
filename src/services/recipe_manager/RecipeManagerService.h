...
struct Pos3Hash {
    size_t operator()(const Pos3Key& k) const noexcept {
        ...
        h = h * 0x9e3799b97f4a7c15ULL + static_cast<uint64_t>(static_cast<uint32_t>(k.y));
        h = h * 0x9e3799b97f4a7c15ULL + static_cast<uint64_t>(static_cast<uint32_t>(k.z));
...
private:
    void onRouterMessage(const std::string& topic, const std::vector<uint8_t>& data);
    ...
    void publishResponse(const std::string& replyTopic, const std::vector<uint8_t>& data);
