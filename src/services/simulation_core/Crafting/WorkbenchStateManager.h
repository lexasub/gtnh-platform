...
public:
    WorkbenchStateManager(std::shared_ptr<simcore::EntityStateStoreClient> essClient,
                          int32_t dimension);
    ...
    std::vector<RecipeManager::ItemStack> getGridState(uint64_t entityId,
                                                        int32_t x, int32_t y, int32_t z);
    // Remove state when workbench is destroyed
    void removeGridState(uint64_t entityId, int32_t x, int32_t y, int32_t z);

private:
    // Serialize 9-item grid to 45-byte buffer: [item_id:uint16, count:uint8, metadata:uint16] x 9
    std::vector<uint8_t> serializeGrid(const std::vector<RecipeManager::ItemStack>& grid) const;
    // Deserialize 45-byte buffer back to 9 ItemStacks
    std::vector<RecipeManager::ItemStack> deserializeGrid(const std::vector<uint8_t>& data) const;
