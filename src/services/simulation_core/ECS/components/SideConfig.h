...
inline constexpr const char* sideRoleName(SideRole role) {
    switch (role) {
        case SideRole::INPUT:     return "INPUT";
        case SideRole::OUTPUT:    return "OUTPUT";
        case SideRole::ENERGY:    return "ENERGY";
        case SideRole::FLUID_IN:  return "FLUID_IN";
        case SideRole::FLUID_OUT: return "FLUID_OUT";
        case SideRole::ANY:       return "ANY";
        case SideRole::NONE:      return "NONE";
        default:                  return "UNKNOWN";
    }
}

// Cycle to next role (GTNH wrench interaction style)
inline constexpr uint8_t nextSideRole(uint8_t current, bool hasFluid, bool hasEnergy) {
    switch (current) {
