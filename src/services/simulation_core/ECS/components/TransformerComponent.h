...
struct TransformerComponent {
    ...
    TransformerComponent(uint8_t inTier, uint8_t outTier, bool up,
                         int32_t buf, int32_t maxIn, int32_t maxOut)
        : inputTier(inTier), outputTier(outTier), stepUp(up),
          buffer(buf), maxInput(maxIn), maxOutput(maxOut) {}
};
