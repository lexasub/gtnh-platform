...
struct EnergyStorage {
  ...
  // Full constructor
  EnergyStorage(int32_t cap, int32_t cur, int32_t maxIn, int32_t maxOut,
                int32_t t)
      : capacity(cap), current(cur), maxInput(maxIn), maxOutput(maxOut),
        tier(t) {}

  // Full constructor with energy type
  EnergyStorage(int32_t cap, int32_t cur, int32_t maxIn, int32_t maxOut,
                int32_t t, EnergyType energy_type)
      : capacity(cap), current(cur), maxInput(maxIn), maxOutput(maxOut),
        tier(t), type(energy_type) {}
