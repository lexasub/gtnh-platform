...
public:
    ...
    using LoadEntityStateCallback = std::function<void(const EntityStateData& state)>;
    void LoadEntityState(int32_t dimension, int32_t x, int32_t y, int32_t z, uint16_t entity_type, LoadEntityStateCallback callback);
    
    // RPC: SaveEntityState
    using SaveEntityStateCallback = std::function<void(bool success)>;
    void SaveEntityState(int32_t dimension, int32_t x, int32_t y, int32_t z, uint16_t entity_type, const std::vector<uint8_t>& stateData, SaveEntityStateCallback callback);
