...
namespace simcore {
class SimulationEngine; class PlayerInventoryStore; class EntityStateStoreClient;
class IEventPublisher; class IoUringRouterClient;
class MachineSlotHandler : public ITopicHandler {
public:
    MachineSlotHandler(std::shared_ptr<SimulationEngine> engine, std::shared_ptr<PlayerInventoryStore> inv,
                       std::shared_ptr<EntityStateStoreClient> ess, std::shared_ptr<IEventPublisher> events,
