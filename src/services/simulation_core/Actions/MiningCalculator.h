...
constexpr uint8_t getBlockHardness(uint16_t block_id) {
  if (block_id == ItemId::pack("0:0:4"))
    return 0; // glass
  if (block_id == ItemId::pack("0:10:0"))
    return 1; // oak_planks
  if (block_id == ItemId::pack("0:0:1"))
    return 2; // stone
  if (block_id == ItemId::pack("0:0:2"))
    return 2; // cobblestone
  if (block_id == ItemId::pack("0:0:5"))
    return 3; // obsidian
  if (ItemId::category(block_id) == ItemId::CAT_ORES)
    return 3;
  return 1; // default
}
...
constexpr uint8_t getBlockMiningLevel(uint16_t block_id) {
  if (block_id == ItemId::pack("10:9"))
    return 3; // diamond_ore — needs HV
  if (block_id == ItemId::pack("10:8"))
    return 2; // lapis_ore — needs MV
  if (block_id == ItemId::pack("10:7"))
    return 2; // redstone_ore — needs MV
  if (block_id == ItemId::pack("10:1"))
    return 2; // gold_ore — needs MV
