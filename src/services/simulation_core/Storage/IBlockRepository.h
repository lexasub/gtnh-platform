...
public:
  ...
  virtual void setBlockCAS(int32_t x, int32_t y, int32_t z,
                           uint16_t expected_block_id, uint16_t new_block_id,
                           uint8_t meta, SetBlockCASCallback callback) = 0;

  using GetBlockCallback = std::function<void(const BlockData &)>;
  virtual void getBlock(int32_t x, int32_t y, int32_t z,
                        GetBlockCallback callback) = 0;
};
