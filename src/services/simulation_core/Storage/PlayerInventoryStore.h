...
using InventoryChangeCallback = std::function<void(
    uint64_t player_id, uint16_t slot_index,
...
public:
	...
	using PostMutationCallback = std::function<void(uint64_t player_id, const std::array<PersistSlot, kInventorySlots>&)>;
	void setPostMutation(PostMutationCallback cb) { postMutation_ = std::move(cb); }
    ...
    /// Calls onChange_ for each changed slot, then for the full set (slot=0xFFFF).
    void setSlots(uint64_t player_id, const std::array<PersistSlot, kInventorySlots>& slots);
    ...
    bool giveItem(uint64_t player_id, uint16_t item_id, uint8_t count, int32_t target_slot = -1);

    // ── FlatBuffer helper ----------------------------------------------------

    /// Build a `Protocol::InventoryUpdate` FlatBuffer from the player's slots.
    flatbuffers::Offset<Protocol::InventoryUpdate> buildUpdate(
...
private:
	...
	std::unordered_map<uint64_t, std::array<PersistSlot, kInventorySlots>> inventories_;
};
