...
struct ChunkMesh {
  ...
  void DestroyGpuBuffers() {
    ...
    if (bgfx::isValid(ib))
      bgfx::destroy(ib);
    if (bgfx::isValid(transparentVb))
      bgfx::destroy(transparentVb);
    if (bgfx::isValid(transparentIb))
      bgfx::destroy(transparentIb);
...
public:
  ...
  // Thread-safe: returns true if chunk at coord already has a mesh for this
  // hash.
  ...
  void ApplyMeshData(ChunkMeshBuilder::MeshData &&data, const ChunkCoord &coord,
                     uint64_t contentHash = 0);

  template <typename F> void ForEachChunk(F &&visitor) {
...
private:
  struct Uint64Hash {
    ...
    static bool equal(uint64_t a, uint64_t b) { return a == b; }
  };
