...
// Manages chunk loading/unloading, provides block queries, integrates with network.
...
public:
    ...
    // TODO(diff-protocol): compile-time option — how we handle block placement feedback:
    //   #define WORLD_FAST_SEND       — send PlayerAction immediately, no waiting
    //   #define WORLD_OPTIMISTIC_AFTER — apply pending block locally before server ack
    //   #define WORLD_SERVER_PRIORITY — server values always win over pending
    ...
    // Atomically check + mark pending, then send request. Returns true if request was sent.
    ...
    void ForEachLoadedChunk(F&& fn) const {
        storage_.ForEachCoord([&](ChunkCoord coord, const std::shared_ptr<const ChunkView>&) {
            fn(coord);
        });
    }

    // Called by NetClient when chunk data arrives
    std::shared_ptr<const ChunkView> OnChunkData(std::shared_ptr<ChunkView> chunk, const ChunkCoord &coord);

    // Called when server sends an incremental diff (BlockUpdate).
    // Applies the change directly to the chunk (server data wins).
    void OnBlockUpdate(BlockPos pos, uint16_t block_id, uint8_t meta, uint32_t mb_id);

    // Called when server confirms our PlayerAction is committed (ACCEPTED or CONFLICT).
    // Removes the position from pending block actions.
    ...
    // Returns true if coord is in pendingRequests_ (chunk requested but not yet received).
    bool IsPending(const ChunkCoord& coord) const;

    std::vector<ChunkCoord> TakeEvictedChunks();

    static float calcCandidateScore(glm::vec3 vec, glm::vec3 vec3, glm::vec3 forward, glm::vec3 velocity);

private:
    void EvictFarChunks(glm::vec3 cameraPos);

    struct Uint64Hash {
        static size_t operator()(uint64_t key) {
            return key;
        }
        static bool operator()(uint64_t a, uint64_t b) {
            return a == b;
        }
    };
    using PendingSet = tbb::concurrent_unordered_set<uint64_t, Uint64Hash>;
    using BlockActionMap = tbb::concurrent_hash_map<uint64_t, int64_t>; // key → timestamp_ns
    ChunkStorage storage_;
    PendingSet pendingRequests_; // chunks requested but not yet received
    BlockActionMap pendingBlockActions_; // block positions with in-flight break/place actions
    ...
    //   - On ChunkData (full snapshot): rebase — apply snapshot, then overlay pendingChanges_
    //   - On BlockUpdate: apply server value, leave pending alone
    //   - On BlockAck: erase from pendingChanges_
    //   - GetBlockAt() returns pendingChanges_ value if present (optimistic render)
};