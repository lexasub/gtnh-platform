...
void ChunkView::SetBlock(int x, int y, int z, uint16_t block_id, uint8_t meta,
    ...
    // Update flat export arrays in place if they exist. They must NOT be
    // reset: ChunkMeshBuilder holds raw pointers into them for the duration
    // of a TBB mesh build (obtained once in ChunkNeighborCache::Init).
    // Regenerating on every SetBlock would also re-allocate + re-copy
    // 96 KB per block edit — the dominant cost of a pipe toggle rebuild.
    if (flat_blocks_) {
        int idx = (y << 10) | (z << 5) | x;
        flat_blocks_[idx] = block_id;
        flat_meta_[idx] = meta;
        flat_mb_[idx] = mb_id;
    }
}
