...
public:
  ...
  std::shared_ptr<const ChunkView>
  StoreAndGetChunk(const ChunkCoord &c, std::shared_ptr<ChunkView> chunk);
  void Clear();

  size_t Size() const;

  template <typename F> void ForEachCoord(F &&fn) const {
...
private:
  ...
  using ChunkMap =
      tbb::concurrent_hash_map<uint64_t, std::shared_ptr<ChunkView>,
                               Uint64Hash>;
  ChunkMap chunks_;
};