...
//
// Note: mech_ and related methods are deprecated but kept for backward compatibility.
// Machine type and energy type are now resolved at runtime via MachineRegistry.
...
public:
    ...
    // ── Public API ──────────────────────────────────────────────────────
    void onMachineSlotAck(uint32_t x, uint32_t y, uint32_t z, uint8_t slotIdx, bool success);
