...
/// и CraftingGrid.
///
/// Состояния: Idle → Holding (после pick up) → Idle (после drop/merge/swap/cancel).
///
/// Использование:
...
///   4. Вызвать RenderPreview каждый кадр для отрисовки перетаскиваемого предмета
///
...
public:
    ...
    /// Начать внешний drag (из CraftingGrid или другого не-инвентарного источника).
    ...
    using ActionCallback = std::function<void(uint8_t actionType,
                                              uint8_t sourceSlot,
                                              uint8_t targetSlot,
                                              uint8_t count)>;
