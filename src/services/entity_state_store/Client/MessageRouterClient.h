...
namespace entity_state_store {
    ...
    private:
        ...
        bool onReadFrame(asio::error_code ec2, uint8_t msg_type, uint32_t data_len, std::shared_ptr<std::vector<uint8_t>> payload);
        ...
        void publishToTopic(const std::string& topic, const std::vector<uint8_t>& payload);
        void handleMessage(const std::string& topic, const std::vector<uint8_t>& data);

        static std::vector<uint8_t> makeFrame(RouterMsg msgType, const std::vector<uint8_t>& payload);
        static std::vector<uint8_t> makePublishFrame(const std::string& topic, const std::vector<uint8_t>& payload);
        static std::vector<uint8_t> makeRegisterFrame(const std::string& serviceName, const std::vector<std::string>& topics);
        ...
        static std::vector<uint8_t> decodePublishFrame(const std::vector<uint8_t>& frame);
