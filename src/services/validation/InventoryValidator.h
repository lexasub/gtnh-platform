...
public:
   ...
   */
  static std::string ValidateItemStack(uint16_t itemId, uint8_t count,
                                       uint16_t meta) {
   ...
   */
  static std::string
  ValidateInventorySlot(const Protocol::InventorySlot *slot) {
    if (!slot) {
   ...
   */
  static std::string
  ValidateInventoryType(const Protocol::InventoryType *inventoryType) {
    if (!inventoryType) {
    ...
    case 2: // Workbench inventory
      if (inventoryType->slot_count() != 10) {
        return "Workbench inventory must have exactly 10 slots (9 crafting + 1 "
               "result)";
      }
      break;
   ...
   */
  static std::string
  ValidateEntityState(const Protocol::EntityState *entityState) {
    if (!entityState) {
    ...
    if (entityState->state()->size() > 0) {
      auto verifier = flatbuffers::Verifier(entityState->state()->data(),
      ...
      auto inventory = flatbuffers::GetRoot<Protocol::Inventory>(
