...
public:
  ...
  void Shutdown();
  void NewFrame(int width, int height, float dt, double mouseX, double mouseY,
                double scrollY, const bool mouseButtons[3], bool mouseCaptured,
                const std::array<bool, 512> &keys, int charCount,
                const uint32_t *charCodepoints);
  void Render();
