...
int main(int argc, char** argv) {
    ...
    RecipeManager::ItemRegistry::instance().loadFromCSV(dataDir + "/src/content/data/registry/items.csv");

    auto recipes = std::make_shared<RecipeManager::RecipeManager>();

    // Load machine definitions from YAML (class→variant mapping)
    if (recipes->loadMachinesFromYaml(dataDir + "/src/content/data/registry/machines.yaml")) {
    ...
    if (recipes->loadRecipesFromYamlDirectory(dataDir + "/src/content/data/recipes/")) {
