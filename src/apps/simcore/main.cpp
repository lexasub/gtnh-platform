...
#include <game/machines/CoolantSystem.h>
...
#include "../../engine/utils/metrics_util.h"
...
int main(int argc, char* argv[]) {
    ...
    const char*  recipes_dir     = (argc > 5) ? argv[5] : "/home/su/src/local/gtnh-platform/src/content/data/recipes";
    const char*  machines_yaml   = (argc > 6) ? argv[6] : "src/content/data/registry/machines.yaml";
    ...
    // ── Item registry (from items.csv → packed ids) ──────────────────────
    {
        ...
        if (reg.loadFromCSV("src/content/data/registry/items.csv")) {
    ...
    {
        auto blockDrops = simcore::BlockDrops::Load("src/content/data/registry/drops.csv");
    ...
    // ── Block transforms (transforms.csv → placed X on Y → Z) ─────────────
    {
        auto blockTransforms =
            simcore::BlockTransforms::Load("src/content/data/registry/transforms.csv");
    ...
    const bool registryLoaded = sharedRegistry.load("src/content/data/registry");
    if (!registryLoaded) {
    ...
    if (!questData->LoadCSV("src/content/data/quests/quests.csv")) {
        spdlog::warn("Failed to load quests.csv");
    }
    if (!questData->LoadGraph("src/content/data/quests/quest_graph.json")) {
        spdlog::warn("Failed to load quest_graph.json");
    }
    if (!questData->LoadRequirementsJSON("src/content/data/quests/quest_requirements.json")) {
