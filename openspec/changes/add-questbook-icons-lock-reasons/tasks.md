...
- [ ] 3.4 Unit tests: auto_complete true/false/default; machine quest completes on output take; fallback path — **simcored_test unverified since QuestManager/MachineSlotHandler edits; must run**
...
- [x] 5.5 Show/hide Complete button per `auto_complete`; hide button + show "Completes automatically" hint for `auto_complete: true`; show button only for non-autoComplete AVAILABLE quests

## 6. Verification
- [x] 6.1 `cmake-build-debug` ninja build passes (`gameclientd` `simcored_exec`)
- [ ] 6.2 `ctest --output-on-failure -j$(nproc)` passes (including new quest_lib + QuestManager tests) — **must run full ctest**
