...
### `craft` — Flat recipe output item ID

`detect_target` = the **flat recipe output item id** from `data/recipes/*.yaml` (`outputs[].item`). This is the id `CraftRequestHandler` reports as `result.item_id` after a successful craft, which `QuestManager::checkCraftCompletion()` compares against via `std::to_string(itemId)`.

**Example mappings** (verified against `data/recipes/crafting_table.yaml`):

| Item | recipe output id | detect_target | Used in quest |
|------|------------------|---------------|---------------|
| wooden_pickaxe | 33 | 33 | Quest 1 |
| iron_pickaxe | 35 | 35 | Quest 3 |
| crafting_table | 14 | 14 | Quest 4 (and block quest 38) |
| furnace | 36 | 36 | Quest 5 (and block quest 37) |
| chest | 37 | 37 | Quest 6 (and block quest 39) |
| iron_ingot | 4 (furnace.yaml output) | 4 | Quest 2 |

> ℹ️ Item ids are **flat recipe ids**, NOT the hierarchical registry ids in `data/registry/items.csv` (e.g. registry `0:10:11:0` = chest, but recipe id 37). Machines with hierarchical registry ids (`1110:xx:y`) are excluded from craft detection — their flat ids may not exist in recipe outputs.
